...
function Application(model)
{
    ...
    function defaultHeaderProcessFactory(model, app, page)
    {
        ...
        if (model.header.indexOf('${') < 0)
            return function(req, res) { res.write(model.header); };				var pages = null;				if (page.breadcrumb)		{			pages = [];						page.breadcrumb.forEach(function(name) {				pages.push(getItemByName(model.pages, name));			});		}
            
        return function(req, res) {
            var header = model.header
                .replace(/\${pagetitle}/g, page.title)
                .replace(/\${sitetitle}/g, model.title)
                .replace(/\${menu}/g, menutext);							var breadcrumb = '';							if (pages)			{				var links = [];				breadcrumb = "";				var npages = 0;								pages.forEach(function(page) {					if (npages)						breadcrumb += " > \n";					var url = expandLink(page.url, req.params);					breadcrumb += "<a href='" + url + "'>" + page.title + "</a>";					npages++;				});			}			
			header = header.replace(/\${breadcrumb}/g, breadcrumb);                
            res.write(header);
        };
    }
